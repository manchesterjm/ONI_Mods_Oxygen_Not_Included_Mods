...
namespace CustomizeYourPaints
{
    ...
    // It switches out the artable currentStage id to some vanilla ID just before saving,
    // and then reverts it once saving is complete.
    public class ArtOverrideRestorer : KMonoBehaviour
    {
        ...
        public void Restore()
        {
            if (!artOverride.overrideStage.IsNullOrWhiteSpace() && artable != null)
            {
                ...
                Logs.Log("Restored custom painting: " + artable.CurrentStage);
            }
        }
        ...
        public void OnSaveGame()
        {
            if (!artOverride.overrideStage.IsNullOrWhiteSpace() && artable != null)
            {
                Logs.Log("Saving: replacing " + artable.CurrentStage + " with fallback " + fallback);
